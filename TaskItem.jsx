...
const TaskItem = ({ task, onToggle, onDelete }) => {
  ...
  return (
    <motion.div
      ...
      className={`group glass flex items-center gap-4 p-5 rounded-3xl transition-all border border-white/10 hover:border-white/30 relative overflow-hidden ${task.completed ? 'opacity-60 bg-white/5' : ''} ${isDueSoon ? 'border-rose-500 bg-rose-500/10 shadow-[0_0_20px_rgba(244,63,94,0.15)]' : ''}`}
    >
      {isDueSoon && (
        <div className="absolute top-0 left-0 w-1.5 h-full bg-rose-500 animate-pulse shadow-[0_0_15px_#f43f5e]"></div>
      )}
      <button className="text-white/20 cursor-grab active:cursor-grabbing">
      ...
      >
        {task.completed ? (
          ...
          <Circle className="w-6 h-6 text-white/30 group-hover:text-white/60 transition-colors" />
        )}
      </button>

      <div className="flex-1 min-w-0">
        <h3 className={`text-base md:text-xl font-black tracking-tight mb-2 ${task.completed ? 'line-through text-white/30' : 'text-white'}`}>
          {task.text}
        </h3>
        ...
        <div className="flex flex-wrap items-center gap-2 mb-3">
          ...
          {task.dueDate && (
            <span className={`text-[11px] flex items-center gap-2 px-4 py-2 rounded-xl font-black tracking-tight border ${isOverdue ? 'text-rose-400 border-rose-500/30 bg-rose-500/10' : isDueSoon ? 'text-rose-600 border-rose-500/50 bg-rose-500/15' : 'text-blue-400 border-blue-500/20 bg-blue-500/5'}`}>
      ...
      <button 
        onClick={onDelete}
        className="opacity-0 group-hover:opacity-100 p-2 hover:bg-rose-500/20 hover:text-rose-400 text-white/30 rounded-xl transition-all active:scale-95"
      >
